...
;(function(){
    ...
    const uw=(typeof unsafeWindow!=='undefined')?unsafeWindow:window,
          myxhr=(typeof GM_xmlhttpRequest === 'function')?GM_xmlhttpRequest:(typeof GM === 'object' && GM !== null && typeof GM.xmlHttpRequest === 'function')?GM.xmlHttpRequest:null,
          info=uw.info=(typeof GM_info!=='undefined'? GM_info:(typeof GM==='object'&&GM !==null&&typeof GM.info==='object'?GM.info:{script:{name:'Userscript',namespace:'Nonamespace'}}));
    init(uw);
    function init(win){
        win.my=new My(win);
        if(top!==self){//iframe
            if(location.protocol.includes('http')){
                win.myaddbtns(function 删(){top.postMessage({'删':location.href},'*');});// 向顶级窗口发送消息
                win.myaddas(location.href);
                win.mywaitele('video',5000).then(v=>{//视频找到
                    v.pause();
                    const rate=win.mybtns.myappend(`input`,'','placeholder','倍速','value',win.mygv('playbackRate',1)).ael("click",e=>{
                        const n=Number(prompt('playbackRate：',rate.value));
                        if(n>0){
                            v.playbackRate=e.target.value=n;
                            win.mysv('playbackRate',n);
                        }
                    }),eles=win.dqa('*');let fs;
                    v.playbackRate=rate.value;
                    for (const ele of eles) {
                        for (const attr of ele.attributes)if (attr.value ==="全屏") fs=ele;
                    }
                    v.onplay=()=>{
                        v.playbackRate=rate.value;
                        if(!win.document.fullscreen)fs.click();
                    }
                    win.document.ael('keydown',function (ev) {//快进20s
                        if (ev.key ===']')v.currentTime+=20;
                        else if (ev.key ==='[')v.currentTime-=20;
                    });
                    win.mywait(1000).then(()=>{v.play();});
                }).catch(err=>{
                    win.cl(err+win.document.URL);
                });
            }
        }
    }
    function My(win){//构造函数，需配合实例方法，与this，prototype有关
        ...
        function getTallestScrollable(node){// 获取点击位置的冒泡路径,并找出最高元素
            let path = [],maxHeight = 0,target;// 从触发事件的元素开始向上遍历父元素
            while(node) {
                path.push(node);
                node = node.parentElement;
            }
            path.forEach(el => {
                const height = el.offsetHeight;// 使用offsetHeight获取可视高度（含内边距/边框）
                if(height > maxHeight) {
                    maxHeight = height;
                    target = el;
                }
        ...
        function eventPreventDefault(ev){
            ev.preventDefault();// 阻止所默认行为
            // event.stopPropagation();阻止事件冒泡到父元素
        }
        function ePDButton1(ev){
        ...
        win.mygv=function(name, defvalue) { //查1
        ...
        win.myms2bjs=timestamp=>{//myms2bjs(Date.now())
            const date=new Date(timestamp),year=date.getFullYear(),month=String(date.getMonth()+1).padStart(2,'0'),day=String(date.getDate()).padStart(2,'0'),hours=String(date.getHours()).padStart(2,'0'),minutes=String(date.getMinutes()).padStart(2,'0');//格式化日期时间
        ...
        win.myaddas=(...urls)=>urls.forEach(async(url,index)=>{//必须配合@connect
            await myxhr({url:url}).then(resp=>resp.responseText).then(html=>{//myaddas("https://www.bilibili.com/","https://www.example.com/")
                const doc=new DOMParser().parseFromString(html,'text/html'),title=document.title;
                win.mybtns.myappend('a',title,'href',url,`className`,'my-a my-btn');
            }).catch(err=>{//跨域了
                cl('跨域了'+err)
                win.mybtns.myappend('a',`框${index}`,'href',url,`className`,'my-a my-btn');
            });
        });
        document.ael("keydown",ev=>{// ev.stopPropagation();// 阻止事件冒泡,开启有坑
            if(ev.altKey){
                const k=ev.key.toLowerCase();
                if (k==='n') win.myz.style.display==='none'?win.myz.style.display='initial':win.myz.style.display='none';//show or hide elements
                else if (k==='m') win.myz.qsa('style').forEach(e=>{e.disabled =!e.disabled});//enable or disable style
                else if (k==='p') {
                    document.getSelection().removeAllRanges()
                    win.myul.style=`position:fixed;left:50%;top:0;display:grid`;
                    const x=innerWidth-getComputedStyle(win.mybtns).width.match(/\d+/)[0],y=innerHeight-getComputedStyle(win.mybtns).height.match(/\d+/)[0];
                    // cl(x,y,innerWidth,innerHeight)
                    win.mybtns.style.transform=`translate(${x}px,${y}px)`;
                    mysv('myBtnsTransform',win.mybtns.style.transform);
                };
            }
        });
        ...
        win.findElementsByInnerHTML=function (text){//按照文本找元素群
            const eles = Array.from(win.dqa('*')).filter(el =>el.innerHTML===text);
            return eles;
        }
        const doc=document,head=document.documentElement.firstElementChild,body=document.body,mysv=win.mysv,mygv=win.mygv,
              myBtnsTransform=mygv('myBtnsTransform','translate(0,0)');
        // alert(myBtnsTransform)
        if(head&&head.tagName==='Z'){
            win.myz=head;
            win.mybtns=head.qs('.my-btns');
        }else{
            win.al=win.alert;win.cl=win.console.log;win.cc=console.clear;win.ce=console.error;
            win.dq=selector=>document.qs(selector);win.dqa=selector=>document.qsa(selector);
            win.si=setInterval;win.st=setTimeout;
            win.myz=head.mybefore('z','');//myzone;rwf纯标识用
            win.myas(`.my-btns{
                      display:block;z-index:9195129;position:fixed;height: min-content;margin: 0;padding: 0;font-family: KaiTi;
                  }.my-btn{
                      max-width:50px;user-select: none;
                  }.my-a{
                      border: 1px solid black!important;font-size: medium;
                  }#swal2-html-container,.swal2-popup,.swal2-toast,.swal2-show {
                      font-size: 15px;margin: 0!important;padding: 0!important;
                  }`);
            win.mybtns=win.myz.myappend('div','',`className`,'my-btns','style',`transform:${myBtnsTransform}`).ael('mouseup',ev=>{
                // alert(win.mybtns.style.transform)
                mysv('myBtnsTransform',win.mybtns.style.transform);
            });
            if(!mygv('isFixBar',0)||win.top!==win.self) win.myBindDrag(win.mybtns);//如果不固定，或在iframe
            if(!mygv('isDocContextmenu',1)) document.ael("contextmenu",eventPreventDefault);//默认右键菜单
            // if(mygv('isContentEditable')) body.contentEditable=true;
            if(!mygv('isDocKeydown')) document.ael("keydown",eventPreventDefault);

            win.mybtns.myappend(`button`,'X','value','X',`className`,"my-btn")
                .myafter('input','','value','顶','type','button',`className`,'my-btn','title','左键一次,中键连续,右键优先连续')
                .myafter('input','','value','底','type','button',`className`,'my-btn','title','左键一次,中键连续,右键优先连续')
                .myafter(`button`,'显',`value`,'显',`className`,"my-btn my-lr",'title','中键全<p>规范化;右键说明')
                .myafter('input','','value','谷','type','button',`className`,'my-btn','title','全页翻译',`style`,'background-image:url("http://translate.google.com/favicon.ico")');
            win.mybtns.ael('mouseup',(async function (ev){//case中不能有async,反之可以
                const v=ev.target.value;
                switch(v){
                    case 'X':win.myz.remove();break;
                    case '顶':{
                        if(win.mysi){
                            win.mysi=''&clearInterval(win.mysi);
                            return new swal({text:'停止连续滚动',timer:1000,toast:true,showConfirmButton:true});
                        }else if(ev.button===2){//右,连续
                            win.mysi=setInterval(async function(){
                                scrollTo(0,0);
                                scrollBy(0,100);
                            },500);
                        }else{
                            const block=v==='顶'?"start":"end",pos1=pageYOffset,
                                  scroll1=win.scrollTo(0,v==='顶'?0:body.scrollHeight),scroll12=body.scrollIntoView({block:block}),pos2=pageYOffset;//await mywait(1000)配合async延时
                            if(pos1===pos2){
                                new swal({text:'没动,请点击滚动区',timer:3000,toast:true,showConfirmButton:true,position:'top-end'});
                                const f = function(event){
                                    const target=getTallestScrollable(event.target);
                                    new swal({title:`target found`,text:`${target.tagName}#${target.id}.${target.className.replaceAll(' ','.')}?`,icon: "question",
                                              toast:true,position:'top-end',timer:3000,timerProgressBar:true,showConfirmButton:true,showCancelButton:!1}).then(result=> {
                                        if(ev.button===0){//左
                                            target.scrollIntoView({ block:block});
                                        }else if(ev.button===1){//中,连续
                                            win.mysi=setInterval(async function(){
                                                target.scrollIntoView({ block:block});
                                                await win.mywait(500);
                                ...
                                body.ael('click',f);
                                await win.mywait(3000);
                                body.rel('click',f);//3s后移除
                            }
                        }break;
                    };
                    case '底':{
                        if(win.mysi){
                            win.mysi=''&clearInterval(win.mysi);
                            return new swal({text:'clearInterval',timer:1000,toast:true,showConfirmButton:true});
                        }else if(ev.button===2){//右,连续
                            win.mysi=setInterval(async function(){
                            ...
                            if(pos1===pos2){
                                ...
                                const f = function(event){
                                    const target=getTallestScrollable(event.target);
                                    new swal({title:`target found`,text:`${target.tagName}#${target.id}.${target.className.replaceAll(' ','.')}?`,icon: "question",
                                              toast:true,position:'top-end',timer:3000,timerProgressBar:true,showConfirmButton:!1,showCancelButton:!1}).then(result=> {
                                        if(ev.button===0){//左
                                            target.scrollIntoView({ block:block});
                                        }else if(ev.button===1){//中,连续
                                            win.mysi=setInterval(async function(){
                                ...
                                body.ael('click',f);
                                await win.mywait(3000);// 不能用mywait(3000).then(body.rel('click',f));顺序会错
                                body.rel('click',f);//3s后移除
                            }
                        }break;
                    };
                    case '显':
                        if(ev.button===0){
                            document.getSelection().removeAllRanges()
                            win.myul.style=`position:fixed;left:50%;top:0;display:grid`;
                            if(!mygv('isFixBar',0)||win.top!==win.self){
                                const x=body.offsetWidth-parseFloat(getComputedStyle(win.mybtns).width),y=innerHeight-parseFloat(getComputedStyle(win.mybtns).height);
                                // cl(x,y,outerWidth,innerHeight)
                                win.mybtns.style.transform=`translate(${x}px,${y}px)`;
                                mysv('myBtnsTransform',win.mybtns.style.transform);
                            }
                        }else if(ev.button===1){//中键全<p>规范化
                            dqa('p').forEach(p=>{p.innerHTML=p.innerText})
                        }else if(ev.button===2){//右键说明
                            new swal({html:'Alt+左键a/右键img<br>Alt+N-显/隐myz中tag<br>Alt+M-开/关myz中style的tag<br>Alt+P-显',
                                      toast:true,showConfirmButton:true,position:'top-end'});
                        }
                        break;
                    case '谷':open('https://translate.google.com/translate?sl=auto&tl=zh-CN&u='+encodeURI(location));break;

                }
            }));
            const switchSelect=win.mybtns.myappend(`select`,'',`className`,"my-btn").ael("click",ev=>{
                switch(ev.target.innerText){
                    case 'isFixBar':{// ev.stopPropagation();//局部阻止浏览器默认右键菜单
                        const oldV=mygv('isFixBar',true),newV=!oldV;
                        mysv('isFixBar',newV)&new swal({title:`isFixBar=${newV}`,text:'刷新后生效',timer:1000,toast:true,position:'top-end',showConfirmButton:false});
                    }break;
                    case 'isDocContextmenu':{//全局开关浏览器默认右键菜单
                        const oldV=mygv('isDocContextmenu',true),newV=!oldV;
                        newV? document.rel("contextmenu",eventPreventDefault):document.ael("contextmenu",eventPreventDefault);
                        mysv('isDocContextmenu',newV)&new swal({title:`isDocContextmenu=${newV}`,timer:1000,toast:true,position:'top-end',showConfirmButton:false});
                    }break;
                    case 'isContentEditable':{
                        const oldV=mygv('isContentEditable',true),newV=!oldV;
                        body.hasAttribute("contentEditable")?body.removeAttribute("contentEditable"):body.contentEditable=true;
                        mysv('isContentEditable',newV)&new swal({title:`isContentEditable=${newV}`,timer:1000,toast:true,position:'top-end',showConfirmButton:false});
                    }break;
                    case 'isDocKeydown':{
                        const oldV=mygv('isDocKeydown',true),newV=!oldV;
                        newV? document.rel("keydown",eventPreventDefault):document.ael("keydown",eventPreventDefault);
                        mysv('isDocKeydown',newV)&new swal({title:`isDocKeydown=${newV}`,timer:1000,toast:true,position:'top-end',showConfirmButton:false});
                    }break;
                    case 'isDocButton1':{
                        const oldV=mygv('isDocButton1',true),newV=!oldV;
                        newV? document.rel("mousedown",ePDButton1):document.ael("mousedown",ePDButton1);
                        mysv('isDocButton1',newV)&new swal({title:`isDocButton1=${newV}`,timer:1000,toast:true,position:'top-end',showConfirmButton:false});
                    }break;
                }
            }).myappend(`option`,'isFixBar').myafter(`option`,'isDocContextmenu').myafter(`option`,'isContentEditable').myafter(`option`,'isDocKeydown').myafter(`option`,'isDocButton1');
        }
        My.divideStr=(str,char)=>Array.from(str).join(char);//以特定字符分隔字符串
        My.reverseStr=str=>str.split('').reverse().join('');//反转str
        My.simplize=txt=>{//输入繁体，输出简体
        ...
        My.findTextNode=o=>{//配合繁转简
            const obj=o.childNodes,olen=obj.length;
            for (let i=0; i<olen; i++) {//遍历搜寻并替换文本节点
                if (obj[i].splitText) obj[i].data=my.simplize(obj[i].data);
                else if (obj[i].childNodes) findTextNode(obj[i]);
            }
        }
        My.getDate=function(){//复制20250324
            let date=new Date(),year=date.getFullYear(),day=date.getDate(),month=date.getMonth()+1;//0-11,0代表1月
